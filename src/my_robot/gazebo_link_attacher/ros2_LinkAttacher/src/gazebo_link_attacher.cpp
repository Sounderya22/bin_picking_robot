...
# #  Date: May, 2023. # # # # ===================================== COPYRIGHT
===================================== #

# ======= CITE OUR WORK ======= #
# You can cite our work with the following statement:
# IFRA-Cranfield (2023) IFRA Gazebo-ROS2 Link Attacher. URL:
...
// IsAttached variable:
// Gazebo breaks if -> An attachment request is done between 2 links, and the
// joint attachment has already been created and not removed! Therefore, we have
// added this variable to make sure the attachment is only requested when the
// previous attachment has already been removed.
...
public:
  ...
  rclcpp::Service<linkattacher_msgs::srv::AttachLink>::SharedPtr
      attach_link_service_;
  rclcpp::Service<linkattacher_msgs::srv::DetachLink>::SharedPtr
      detach_link_service_;

  // getJoint function:
  bool getJoint(std::string M1, std::string L1, std::string M2, std::string L2,
                JointSTRUCT &joint);
};
...
void GazeboLinkAttacher::Load(gazebo::physics::WorldPtr _world,
                              sdf::ElementPtr _sdf) {
...
void GazeboLinkAttacherPrivate::Attach(
  ...
  JointSTRUCT j;
  if (this->getJoint(_req->model1_name, _req->link1_name, _req->model2_name,
  _req->link2_name, j)){ j.joint->Attach(j.l1, j.l2); _res->success = true;
    _res->message = "ATTACHED: {MODEL , LINK} -> {" + _req->model1_name + " , "
  + _req->link1_name + "} -- {" + _req->model2_name + " , " + _req->link2_name +
  "}."; return;
  }
  */
  ...
  if (IsAttached == true) {
    ...
    JointName = _req->model1_name + "_" + _req->link1_name + "_" +
                _req->model2_name + "_" + _req->link2_name + "_joint";
    gazebo::physics::JointPtr joint =
        model1->CreateJoint(JointName, "fixed", link1, link2);
    ...
    _res->success = true;
    _res->message = "ATTACHED: {MODEL , LINK} -> {" + _req->model1_name +
                    " , " + _req->link1_name + "} -- {" + _req->model2_name +
                    " , " + _req->link2_name + "}.";
...
void GazeboLinkAttacherPrivate::Detach(
  ...
  if (this->getJoint(_req->model1_name, _req->link1_name, _req->model2_name,
    ...
    j.joint->Detach();
    _res->success = true;
    _res->message = "DETACHED: {MODEL , LINK} -> {" + _req->model1_name +
                    " , " + _req->link1_name + "} -- {" + _req->model2_name +
                    " , " + _req->link2_name + "}.";

    // (+) Remove joint --> This fixes the following problem: If the object to
    // be attached is removed and spawned again, gazebo breaks when attaching it
    // again, since the joint already existed. Joint must be REMOVED when
    // detaching.
    gazebo::physics::ModelPtr model1 = world_->ModelByName(_req->model1_name);
    ...
    _res->success = false;
    _res->message =
        "DETACHED -- ERROR (Joint does not exist!): {MODEL , LINK} -> {" +
        _req->model1_name + " , " + _req->link1_name + "} -- {" +
        _req->model2_name + " , " + _req->link2_name + "}.";
  }
}

bool GazeboLinkAttacherPrivate::getJoint(std::string M1, std::string L1,
                                         std::string M2, std::string L2,
                                         JointSTRUCT &joint) {
  JointSTRUCT j;
  for (std::vector<JointSTRUCT>::iterator it = GV_joints.begin();
       it != GV_joints.end(); ++it) {
    j = *it;
    if ((j.model1.compare(M1) == 0) && (j.model2.compare(M2) == 0) &&
        (j.link1.compare(L1) == 0) && (j.link2.compare(L2) == 0)) {
      joint = j;
      return true;
    }
  }
