...
public:
  PoseVisualizer() : Node("pose_visualizer") {
    ...
    visual_tools_ = std::make_shared<moveit_visual_tools::MoveItVisualTools>(
        this->shared_from_this(), "base_link", "/moveit_visual_tools");
    visual_tools_->deleteAllMarkers();
    visual_tools_->loadRemoteControl();

    // Set up Move Group Interface
    // move_group_ =
    // std::make_shared<moveit::planning_interface::MoveGroupInterface>(this,
    // "manipulator");
    move_group_ =
        std::make_shared<moveit::planning_interface::MoveGroupInterface>(
            this->shared_from_this(), "ur_manipulator", tf_buffer_);
...
private:
  ...
  void visualizePose() {
    ...
    // target_pose.position.x = 0.5;  // Adjust these values based on your
    // robot's workspace target_pose.position.y = 0.2; target_pose.position.z =
    // 0.3;
    ...
    if (success) {
      ...
      visual_tools_->publishText(Eigen::Isometry3d::Identity(),
                                 "Planning Failed", rviz_visual_tools::RED,
