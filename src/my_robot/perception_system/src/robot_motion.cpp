...
void URMotion::setupCollisionObjects()
{
    ...
    setupCollisionObject("dropbox", DROPBOX_MESH_PATH, mesh_pose, dropbox_collision_object);
...
bool URMotion::setupCollisionObject(const std::string &object_id,
...
bool URMotion::operateGripper(const bool &close_gripper)
{
    ...
    gripper_current_state->copyJointGroupPositions(gripper_joint_model_group_, gripper_joint_positions);
    
    RCLCPP_DEBUG(this->get_logger(), "No. of joints in eef_group: %zd", gripper_joint_positions.size());

    if (close_gripper) {
        gripper_joint_positions[0] = 0.04;  // Adjust based on your robot's gripper configuration
    ...
    bool success = (move_group_interface_->move() == moveit::planning_interface::MoveItErrorCode::SUCCESS);
